...
foreach ($events as $i => $event) {
	...
	if ($participants = getEventParticipants($event->ID)) {
		$args['events'][$i]->participants = $participants;
		...
		foreach ($participants as $participant) {
			...
			foreach ($keys as $key => $value) {
				if (!empty($additional_answers[$key]) && is_array($additional_answers[$key])) {
					foreach ($additional_answers[$key] as $label) {
						$stats[$value['pm_label']][$label]++;
					}
				} elseif (!empty($additional_answers[$key])) {
