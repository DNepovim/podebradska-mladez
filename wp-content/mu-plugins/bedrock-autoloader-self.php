...
class AutoloaderSelf {
  ...
  function __construct($subdirectory = '') {
    if (isset(self::$_single)) { return; }

    self::$_single       = $this; // Singleton set.
    self::$subdirectory  = $subdirectory;
    self::$relative_path = '/../' . basename(__DIR__) . '/' . self::$subdirectory; // Rel path set.
  ...
  public function loadPlugins() {
    ...
    foreach (self::$cache['plugins'] as $plugin_file => $plugin_info) {
      include_once(WPMU_PLUGIN_DIR . '/'. self::$subdirectory . '/' . $plugin_file);
    }
  ...
  private function validatePlugins() {
    foreach (self::$cache['plugins'] as $plugin_file => $plugin_info) {
      if (!file_exists(WPMU_PLUGIN_DIR .'/'. self::$subdirectory . '/' . $plugin_file)) {
        $this->updateCache();
        break;
      }
    }
  ...
  private function countPlugins() {
    ...
    $count = count(glob(WPMU_PLUGIN_DIR .'/'. self::$subdirectory . '/*/', GLOB_ONLYDIR | GLOB_NOSORT));
